/**
 * MT5Chart.tsx
 * Chart candlestick cu date reale din MT5.
 * Pune in: src/components/charts/MT5Chart.tsx
 */
...
export function MT5Chart({ sym, tf, requestCandles, livePrice }: Props) {
  ...
  const draw = useCallback(() => {
    ...
    if (livePrice && livePrice >= minP && livePrice <= maxP) {
      ...
      ctx.strokeStyle = COLORS.priceLine; ctx.lineWidth = 1
      ctx.setLineDash([4, 3])
    ...
    if (m && m.x > PAD.left && m.x < W - PAD.right && m.y > PAD.top && m.y < H - PAD.bottom) {
      ctx.strokeStyle = COLORS.cross; ctx.lineWidth = 1; ctx.setLineDash([3, 3])
      ctx.beginPath(); ctx.moveTo(m.x, PAD.top); ctx.lineTo(m.x, H - PAD.bottom); ctx.stroke()
      ctx.beginPath(); ctx.moveTo(PAD.left, m.y); ctx.lineTo(W - PAD.right, m.y); ctx.stroke()
      ctx.setLineDash([])
      // Pret la crosshair
      const cp = toP(m.y)
      ...
      // OHLC tooltip
      const ci = Math.floor((m.x - PAD.left) / (cw + 2))
      if (ci >= 0 && ci < slice.length) setOhlc(slice[ci])
      else setOhlc(null)
    } else {
      setOhlc(null)
    }
  ...
  // Update ultima lumanare cu pretul live
  useEffect(() => {
  ...
  const onLeave  = () => { mouseRef.current = null; dragRef.current.active = false; setOhlc(null); draw() }
  const onDown   = (e: React.MouseEvent) => { dragRef.current = { active: true, startX: e.clientX, startOff: viewRef.current.offset } }
  const onUp     = () => { dragRef.current.active = false }
  const onWheel  = (e: React.WheelEvent) => {
    e.preventDefault()
    viewRef.current.cw = Math.max(3, Math.min(40, viewRef.current.cw + (e.deltaY > 0 ? -1 : 1)))
    draw()
  }
  ...
  return (
    <div ref={wrapRef} style={{ width: '100%', height: '100%', position: 'relative', background: '#fff' }}>
      ...
      {/* OHLC tooltip */}
      {ohlc && (
        <div style={{ position: 'absolute', top: 8, left: 10, background: 'rgba(26,58,107,.9)', color: '#fff', fontSize: '10px', padding: '5px 9px', borderRadius: '6px', fontFamily: "'JetBrains Mono',monospace", lineHeight: 1.9, pointerEvents: 'none' }}>
          <div style={{ opacity: .6, fontSize: '9px', marginBottom: 1 }}>{fmtTime(ohlc.time, tf)}</div>
      ...
      {error && !loading && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,.9)', gap: 8 }}>
          ...
          <button onClick={load} style={{ padding: '5px 14px', background: '#2255CC', color: '#fff', border: 'none', borderRadius: 6, fontSize: 11, cursor: 'pointer' }}>Incearca din nou</button>
        </div>
      )}

      {/* Badge MT5 */}
      <div style={{ position: 'absolute', top: 8, right: 80, fontSize: 9, color: '#8FA3BF', background: 'rgba(244,247,253,.9)', padding: '2px 7px', borderRadius: 10, border: '1px solid #E8EEF8', fontFamily: 'Inter,sans-serif' }}>
        MT5 · {sym}
      </div>
    </div>
  )
