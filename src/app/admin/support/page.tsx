            ...
            {['open','pending','resolved','closed','all'].map(s => (
              <button key={s} onClick={() => applyFilter(s)}
                className={`px-[10px] py-[5px] text-[8px] tracking-[1.5px] uppercase font-semibold cursor-pointer border transition-all ${
                  filter === s
                    ? 'bg-[rgba(34,85,204,.08)] border-[#C5D5EA] text-[#2255CC]'
            ...
            <div className="py-10 text-center">
              ...
              <div className="text-[11px] text-[#8FA3BF]">
                ...
                {totalCount > 0 && filter !== 'all' && (
                  <span> — <button onClick={() => applyFilter('all')} className="text-[#2255CC] underline cursor-pointer bg-transparent border-none">view all {totalCount}</button></span>
                )}
              </div>
            </div>
          ) : (
              ...
              <tbody>
                {tickets.map(t => (
                  <tr key={t.id} className="border-b border-[rgba(220,38,38,.04)] hover:bg-[rgba(220,38,38,.02)]">
                    ...
                    <td className="px-[11px] py-[8px] text-[10px] text-[#8FA3BF]">
