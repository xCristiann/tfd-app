    ...
    <>
      <DashboardLayout title="Challenge Products" nav={ADMIN_NAV} accentColor="red">
        <div className="flex justify-between items-center mb-4">
          <span className="text-[11px] text-[#8FA3BF]">{products.length} products · {products.filter(p=>p.is_active).length} active</span>
        ...
        {loading ? (
          ...
          <Card><div className="py-16 text-center">
            ...
            <p className="text-[12px] text-[#5C7A9E] mb-4">No challenge products yet.</p>
            ...
            {products.map(p=>(
              <Card key={p.id}>
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <div className="font-sans text-[17px] font-bold text-[#2255CC]">${Number(p.account_size).toLocaleString()}</div>
                    <div className="text-[11px] text-[#5C7A9E]">{p.name}</div>
                    <div className={`text-[9px] mt-1 font-semibold ${p.is_active ? 'text-[#16A34A]' : 'text-[#DC2626]'}`}>
      ...
      <Modal open={modal} onClose={()=>setModal(false)}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 flex flex-col gap-1">
            <label className="text-[7px] tracking-[2px] uppercase text-[#8FA3BF] font-semibold">Type</label>
            <div className="flex bg-[#F4F7FD] border border-[#F0F4FB]">
              {['2step','1step'].map(t=>(
                <button key={t} onClick={()=>setForm((f:any)=>({...f,challenge_type:t}))}
                  className={`flex-1 py-[7px] text-[9px] uppercase font-bold cursor-pointer border-none transition-all ${form.challenge_type===t?'bg-[rgba(34,85,204,.15)] text-[#2255CC]':'bg-transparent text-[#8FA3BF]'}`}>{t}</button>
              ))}
            </div>
          </div>
          <div className="col-span-2 text-[8px] tracking-[2px] uppercase text-[#8FA3BF] font-semibold pt-1">Phase 1</div>
          ...
          {form.challenge_type === '2step' && <>
            <div className="col-span-2 text-[8px] tracking-[2px] uppercase text-[#8FA3BF] font-semibold pt-1">Phase 2</div>
          ...
          <div className="col-span-2 text-[8px] tracking-[2px] uppercase text-[#8FA3BF] font-semibold pt-1">Funded Account</div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button onClick={save} disabled={saving}
            className="px-[22px] py-[8px] bg-[#2255CC] text-[#F0F4FB] text-[9px] tracking-[2px] uppercase font-bold cursor-pointer border-none disabled:opacity-50">
