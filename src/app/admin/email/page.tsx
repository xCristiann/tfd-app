    ...
    <>
      <DashboardLayout title="Email Center" nav={ADMIN_NAV} accentColor="red">
        ...
        <div className="grid grid-cols-3 gap-[11px]">
          ...
          <KPICard label="Sent This Session" value={String(sentLog.reduce((s,l) => s + l.success, 0))} sub="Emails delivered" subColor="text-[var(--green)]"/>
          <KPICard label="Total Traders" value={String(traders.length)} sub="In database" subColor="text-[var(--gold)]"/>
        </div>
                  ...
                  <button key={t.id} onClick={() => selectTemplate(t)}
                    className={`flex items-center gap-3 px-3 py-[9px] text-left cursor-pointer border transition-all ${
                      selectedTemplate?.id === t.id
                        ? 'border-[var(--bdr2)] bg-[rgba(212,168,67,.07)]'
                        : 'border-transparent hover:border-[var(--dim)] hover:bg-[var(--bg3)]'
                    }`}>
                    <span className="text-[16px] flex-shrink-0">{t.icon}</span>
                    <div className="flex-1 min-w-0">
                      <div className={`text-[11px] font-semibold ${selectedTemplate?.id === t.id ? 'text-[var(--gold)]' : ''}`}>{t.label}</div>
                      <div className="text-[9px] text-[var(--text3)]">{t.desc}</div>
                    </div>
                    {t.auto && (
                      <span className="text-[7px] px-[5px] py-[2px] bg-[rgba(0,217,126,.08)] text-[var(--green)] border border-[rgba(0,217,126,.15)] font-bold uppercase tracking-[1px] flex-shrink-0">Auto</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <Card>
                ...
                <div className="flex flex-col gap-2">
                  {sentLog.slice(0, 8).map(l => (
                    <div key={l.id} className="flex items-center justify-between py-[6px] border-b border-[var(--dim)] last:border-0 text-[10px]">
                      <div>
                        <div className="font-semibold">{l.template}</div>
                        <div className="text-[9px] text-[var(--text3)]">{l.time} · {l.recipients} recipient{l.recipients>1?'s':''}</div>
                      </div>
                      <span className={`font-mono font-bold ${l.fail > 0 ? 'text-[var(--gold)]' : 'text-[var(--green)]'}`}>
          ...
          <Card>
            {!selectedTemplate ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-[11px] text-[var(--text3)]">Choose an email template from the left to compose and send.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-3 mb-5 pb-4 border-b border-[var(--bdr)]">
                  <span className="text-[24px]">{selectedTemplate.icon}</span>
                  <div>
                    <div className="font-serif text-[16px] font-bold">{selectedTemplate.label}</div>
                    <div className="text-[10px] text-[var(--text3)]">{selectedTemplate.desc}</div>
                  </div>
                  {selectedTemplate.auto && (
                    <div className="ml-auto text-[10px] text-[var(--green)] bg-[rgba(0,217,126,.08)] border border-[rgba(0,217,126,.15)] px-3 py-1">
                ...
                {/* Recipient */}
                <div className="mb-5">
                  <label className="text-[7px] uppercase tracking-[2px] text-[var(--text3)] font-semibold block mb-2">Recipient</label>
                    ...
                    {[
                      ...
                      <button key={key} onClick={() => setRecipientMode(key as any)}
                        className={`px-3 py-[6px] text-[9px] uppercase font-bold tracking-[1px] cursor-pointer border transition-all ${
                          recipientMode === key
                            ? 'bg-[rgba(255,51,82,.1)] border-[rgba(255,51,82,.3)] text-[var(--red)]'
                  ...
                  {recipientMode === 'single' && (
                    <div>
                      <input
                        ...
                        className="w-full px-3 py-2 bg-[var(--bg3)] border border-[var(--bdr2)] text-[var(--text)] text-[11px] outline-none focus:border-[var(--gold)] transition-colors mb-2"
                      />
                      {traderSearch && (
                        <div className="max-h-[180px] overflow-y-auto border border-[var(--bdr)] bg-[var(--bg3)]">
                          {filteredTraders.length === 0 ? (
                            <div className="px-3 py-4 text-[10px] text-[var(--text3)] text-center">No traders found</div>
                            ...
                            <button key={t.id} onClick={() => { setSelectedTrader(t); setTraderSearch(`${t.first_name} ${t.last_name} — ${t.email}`) }}
                              className={`w-full flex items-center gap-3 px-3 py-[8px] text-left border-b border-[var(--dim)] last:border-0 cursor-pointer hover:bg-[rgba(212,168,67,.06)] transition-colors ${selectedTrader?.id === t.id ? 'bg-[rgba(212,168,67,.06)]' : ''}`}>
                              <div className="w-7 h-7 bg-[rgba(212,168,67,.1)] border border-[var(--bdr2)] flex items-center justify-center text-[10px] font-bold text-[var(--gold)] flex-shrink-0">
                              ...
                              {selectedTrader?.id === t.id && <span className="ml-auto text-[var(--gold)] text-[12px]">✓</span>}
                            </button>
                          ))}
                        </div>
                      )}
                      {selectedTrader && !traderSearch.includes('—') && (
                        <div className="flex items-center gap-2 px-3 py-2 bg-[rgba(212,168,67,.06)] border border-[rgba(212,168,67,.2)]">
                          <span className="text-[var(--gold)] text-[11px] font-semibold">{selectedTrader.first_name} {selectedTrader.last_name}</span>
                          <span className="text-[var(--text3)] text-[10px]">— {selectedTrader.email}</span>
                          <button onClick={() => { setSelectedTrader(null); setTraderSearch('') }} className="ml-auto text-[var(--text3)] hover:text-[var(--red)] cursor-pointer bg-transparent border-none text-[13px]">✕</button>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {recipientMode === 'all' && (
                    <div className="flex items-center gap-3 px-4 py-3 bg-[rgba(255,51,82,.06)] border border-[rgba(255,51,82,.2)]">
                  ...
                  {recipientMode === 'custom' && (
                    <input
                      ...
                      className="w-full px-3 py-2 bg-[var(--bg3)] border border-[var(--bdr2)] text-[var(--text)] text-[11px] outline-none focus:border-[var(--gold)] transition-colors"
                    />
                  )}
                </div>
                ...
                {selectedTemplate.fields.length > 0 && (
                  <div className="mb-5">
                    <div className="text-[7px] uppercase tracking-[2px] text-[var(--text3)] font-semibold mb-3">
                      Email Data {recipientMode === 'single' && selectedTrader && <span className="text-[var(--green)] ml-2">— first_name filled automatically</span>}
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      {selectedTemplate.fields.map((f: string) => (
                        <div key={f} className={f === 'body' || f === 'reply_body' ? 'col-span-2' : ''}>
                          <label className="text-[8px] uppercase tracking-[1px] text-[var(--text3)] font-semibold block mb-1">{FIELD_LABELS[f] ?? f}</label>
                          {f === 'body' || f === 'reply_body' ? (
                            <textarea
                              value={fieldValues[f] ?? ''}
                              ...
                              rows={5}
                              className="w-full px-3 py-2 bg-[var(--bg3)] border border-[var(--bdr2)] text-[var(--text)] text-[11px] outline-none focus:border-[var(--gold)] transition-colors resize-y font-mono"
                            />
                          ) : (
                            <input
                              value={fieldValues[f] ?? ''}
                ...
                {/* Preview note */}
                <div className="p-3 bg-[var(--bg3)] border border-[var(--dim)] mb-5 text-[10px] text-[var(--text3)]">
                  <span className="text-[var(--gold)] font-semibold">Email preview: </span>
