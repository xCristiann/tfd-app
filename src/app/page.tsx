...
import type { TraderStats, DailySnapshot } from '@/types/database'
...
export function DashboardPage() {
  ...
  useEffect(() => {
    if (!primary) return
    analyticsApi.getStats(primary.id).then(setStats).catch(() => {})
    analyticsApi.getEquityCurve(primary.id, 30).then(setCurve).catch(() => {})
  ...
  const profit = primary ? primary.balance - primary.starting_balance : 0
  const profitPct = primary && primary.starting_balance > 0
    ? ((profit / primary.starting_balance) * 100).toFixed(2)
    : '0.00'
  const withdrawable = profit > 0
    ? profit * ((prod?.funded_profit_split ?? 85) / 100)
    : 0
  const dailyLimit = prod?.funded_daily_dd ?? prod?.ph1_daily_dd ?? 5
  const maxLimit   = prod?.funded_max_dd   ?? prod?.ph1_max_dd   ?? 10
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-5 gap-[11px]">
              <KPICard label="Balance"       value={fmt(primary.balance)}  sub={`${profit >= 0 ? '+' : ''}${fmt(profit)} profit`} subColor={profit >= 0 ? 'text-[var(--green)]' : 'text-[var(--red)]'} />
              <KPICard label="Equity"        value={fmt(primary.equity)}   sub="Current equity"         subColor="text-[var(--text2)]" />
              ...
              <KPICard label="Profit Factor" value={stats?.profit_factor ? String(stats.profit_factor) : '—'}
                sub="Target: 1.5+" subColor="text-[var(--green)]" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-[14px]">
              <Card>
                ...
                {curve.length === 0 && (
                  <div className="text-center text-[11px] text-[var(--text3)] py-4">No trading history yet</div>
                )}
              </Card>
              <Card>
                <CardHeader title="Risk Dashboard" />
                <DrawdownBar label="Daily Drawdown" value={primary.daily_dd_used ?? 0} max={dailyLimit} />
                <DrawdownBar label="Max Drawdown"   value={primary.max_dd_used ?? 0}   max={maxLimit} warn={60} danger={80} />
                <div className="mb-[11px]">
                  <div className="flex justify-between mb-[4px]">
                    <span className="text-[9px] tracking-[1.5px] uppercase text-[var(--text3)] font-semibold">Profit Progress</span>
                    <span className={`mono text-[11px] ${parseFloat(profitPct) >= (prod?.ph1_profit_target ?? 8) ? 'text-[var(--green)]' : 'text-[var(--gold)]'}`}>
                      {profitPct}% / {prod?.ph1_profit_target ?? 8}% target
                    </span>
                  </div>
                  <div className="h-[4px] bg-white/5 rounded-[2px] overflow-hidden">
                    <div className="h-full rounded-[2px] bg-[var(--green)] transition-all"
                      style={{ width: `${Math.min((parseFloat(profitPct) / (prod?.ph1_profit_target ?? 8)) * 100, 100)}%` }} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 mt-3">
                  {[
                    ...
                    ['Total P&L',   stats?.total_pnl   != null ? fmt(stats.total_pnl)   : '—', stats?.total_pnl >= 0 ? 'var(--green)' : 'var(--red)'],
                  ].map(([l,v,c])=>(
