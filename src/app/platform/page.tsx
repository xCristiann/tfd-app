...
export function PlatformPage() {
  ...
  const closingRef = useRef<Set<string>>(new Set()) // prevent double-close
  useEffect(() => {
    if (!primary?.id) return
    const iv = setInterval(async () => {
      const trades = tradesRef.current // always current via ref
      ...
      for (const t of trades) {
        ...
        // CRITICAL: only use REAL live price, never SEEDS for SL/TP checks
        const realPrice = priceRef.current[t.symbol]
        if (!realPrice || realPrice <= 0) continue // no real price yet, skip
        ...
        if (t.sl) {
          ...
          if (sl > 0) {
            const slHit = t.direction==='buy' ? cur <= sl : cur >= sl
            if (slHit) hitReason = `SL hit @ ${cur.toFixed(i.dec)}`
          }
        }
        // TP hit check
        if (!hitReason && t.tp) {
          ...
          if (tp > 0) {
            const tpHit = t.direction==='buy' ? cur >= tp : cur <= tp
            if (tpHit) hitReason = `TP hit @ ${cur.toFixed(i.dec)}`
          }
        }
        ...
        try {
          ...
          // Sanity check - max loss/gain per trade can't exceed 2x account balance
          const bal = pr.balance ?? 0
          if (Math.abs(netPnl) > bal * 2) {
            console.error(`[auto-close] Sanity fail: netPnl=${netPnl} bal=${bal} sym=${t.symbol} cur=${cur} open=${t.open_price}`)
            closingRef.current.delete(t.id)
            continue
          }

          const now    = new Date().toISOString()
          const newBal = +(bal + netPnl).toFixed(2)

          await supabase.from('trades').update({ status:'closed', close_price:cp, net_pnl:netPnl, pips, closed_at:now }).eq('id', t.id)
          await supabase.from('accounts').update({ balance:newBal, equity:newBal }).eq('id', pr.id)

          setOpenTrades(p => p.filter(x => x.id !== t.id))
          setClosedTrades(p => [{ ...t, status:'closed', close_price:cp, net_pnl:netPnl, pips, closed_at:now }, ...p])
          toast(netPnl>=0?'success':'error', netPnl>=0?'🎯':'🛑',
            `${hitReason} — ${t.symbol}`,
            `${netPnl>=0?'+':''}$${netPnl.toFixed(2)} | ${pips>=0?'+':''}${pips} pips`)
        } catch(e) {
          closingRef.current.delete(t.id) // allow retry on error
          console.error('[auto-close error]', e)
        }
      }
    }, 1000)
    return () => clearInterval(iv)
  }, [primary?.id]) // don't depend on openTrades - use tradesRef instead
