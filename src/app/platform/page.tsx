...
export function PlatformPage(){
  ...
  useEffect(()=>{try{localStorage.setItem('tfd_sym',sym)}catch{}},[sym])
  useEffect(()=>{try{localStorage.setItem('tfd_tf',tf)}catch{}},[tf])
  ...
  // Force P&L re-render every second so it fluctuates live
  ...
  const openPnl    =openTrades.reduce((s,t)=>s+calcPnl(t,refPrices.current[t.symbol]||prices[t.symbol]||SEED[t.symbol]),0)
  ...
  <div style={{display:'flex',height:'100vh',overflow:'hidden',background:'#0A0A0F',color:'#1A3A6B',fontSize:12}}>
          ...
          return(
            <>
              ...
              {watchlist.favs.length>0&&(
                <div>
                  <div style={{padding:'4px 10px',fontSize:7,letterSpacing:2,textTransform:'uppercase' as const,color:'#2255CC',fontWeight:700,background:'rgba(212,168,67,.06)',borderBottom:'1px solid #E8EEF8',display:'flex',alignItems:'center',gap:5}}>
                    <span>★</span> Favorites
                  </div>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{height:50,flexShrink:0,background:'#fff',borderBottom:'1px solid #E8EEF8',display:'flex',alignItems:'center',padding:'0 14px',gap:10}}>
        ...
        {(primary as any)?.payout_locked&&<div style={{padding:'4px 10px',background:'rgba(212,168,67,.1)',border:'1px solid #C5D5EA',fontSize:9,color:'#2255CC',letterSpacing:1,fontWeight:600,textTransform:'uppercase' as const,flexShrink:0}}>⏳ Payout Pending</div>}
        <div style={{marginLeft:'auto',display:'flex',gap:2}}>
          {Object.keys(TF_CONFIG).map(t=>(
            <button key={t} onClick={()=>setTf(t)} style={{padding:'3px 7px',fontSize:9,fontFamily:'monospace',fontWeight:'bold',cursor:'pointer',background:tf===t?'#2255CC':'#F4F7FD',border:'none',color:tf===t?'#fff':'#5C7A9E',borderRadius:'5px'}}>{t}</button>
          ))}
        </div>
      ...
      <div style={{height:220,flexShrink:0,background:'#fff',borderTop:'1px solid #E8EEF8',display:'flex',flexDirection:'column'}}>
        <div style={{display:'flex',borderBottom:'1px solid #E8EEF8'}}>
          {[['positions',`Positions (${openTrades.length})`],['history',`History (${closedTrades.length})`],['account','Account']].map(([k,l])=>(
            <button key={k} onClick={()=>setTab(k)} style={{padding:'7px 14px',fontSize:9,letterSpacing:1,textTransform:'uppercase' as const,fontWeight:600,cursor:'pointer',border:'none',marginBottom:-1,borderBottom:tab===k?'2px solid #2255CC':'2px solid transparent',background:tab===k?'rgba(212,168,67,.04)':'transparent',color:tab===k?'#2255CC':'#8FA3BF'}}>{l}</button>
          ))}
        </div>
                   ...
                   <tr key={t.id} style={{borderBottom:'1px solid rgba(212,168,67,.04)',background:warn?'rgba(255,51,82,.05)':'transparent'}}>
                     ...
                     <td style={{padding:'5px 8px'}}>
                       {isEditing
                         ?<input value={editSLTP!.sl} onChange={e=>setEditSLTP(x=>x?{...x,sl:e.target.value}:null)}
                             style={{width:70,padding:'2px 4px',background:'rgba(255,51,82,.1)',border:'1px solid rgba(255,51,82,.4)',color:'#DC2626',fontFamily:'monospace',fontSize:9,outline:'none'}}
                     ...
                     <td style={{padding:'5px 8px'}}>
                       {isEditing
                         ?<input value={editSLTP!.tp} onChange={e=>setEditSLTP(x=>x?{...x,tp:e.target.value}:null)}
                             style={{width:70,padding:'2px 4px',background:'rgba(0,217,126,.1)',border:'1px solid rgba(0,217,126,.4)',color:'#16A34A',fontFamily:'monospace',fontSize:9,outline:'none'}}
                     ...
                     <td style={{padding:'5px 8px',display:'flex',gap:4}}>
                       {isEditing
                         ...
                         :<button onClick={()=>closeTrade(t)} style={{padding:'3px 8px',fontSize:8,textTransform:'uppercase' as const,fontWeight:'bold',cursor:'pointer',background:'rgba(255,51,82,.1)',color:'#DC2626',border:'1px solid rgba(255,51,82,.25)'}}>Close</button>
                       }
                     </td>
                   </tr>
                 )
      ...
      <div style={{flex:1,overflowY:'auto',padding:12,display:'flex',flexDirection:'column',gap:8}}>
        ...
        <div>
          ...
          <div style={{display:'flex',background:'#F4F7FD',border:'1px solid rgba(26,58,107,.06)'}}>
            {['Market','Limit','Stop'].map(t=><button key={t} onClick={()=>setOrdType(t)} style={{flex:1,padding:'6px 0',fontSize:8,textTransform:'uppercase' as const,fontWeight:'bold',cursor:'pointer',border:'none',background:ordType===t?'rgba(212,168,67,.12)':'transparent',color:ordType===t?'#2255CC':'#8FA3BF'}}>{t}</button>)}
          </div>
        </div>
        {/* Lot size */}
        <div>
