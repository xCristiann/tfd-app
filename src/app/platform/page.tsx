...
/* ══ TRADINGVIEW WIDGET ═══════════════════════════════════════════ */
function TVChart({ tvSym, tfTv, onPrice }: {
  tvSym: string; tfTv: string; onPrice: (price:number)=>void
}) {
  ...
  useEffect(() => {
    ...
    // Poll TV iframe title/DOM for price every 500ms
    // TV widget sets document.title of iframe to current price
    const priceInterval = setInterval(() => {
      const iframe = wrap.querySelector('iframe') as HTMLIFrameElement|null
      if (!iframe) return
      try {
        // TV widget URL contains symbol - try reading from iframe src
        const src = iframe.src || ''
        // Try postMessage to get quote
        iframe.contentWindow?.postMessage(
          JSON.stringify({ name: 'request-quote', symbol: tvSym }), '*'
        )
      } catch {}
    }, 500)
    ...
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:'#F0F4FB',color:'#1A3A6B',height:'100vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{flex:1}}>
            <TVChart tvSym={inst.tv} tfTv={tf.tv} onPrice={(p)=>push(sym,p)} />
          </div>
        </div>
