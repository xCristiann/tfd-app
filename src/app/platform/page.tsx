...
function calcPnl(trade:any,price:number):number{
  ...
  const isJpy=['USD/JPY','GBP/JPY','EUR/JPY'].includes(trade.symbol)
  const units=isJpy?LOT_SIZE/price:(inst?.lotUSD(1)??LOT_SIZE)
...
function useRiskMonitor(
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{
      ...
      if(equity<=maxDDFloor){firedRef.current=true;cbRef.current(`Max drawdown breached — equity $${equity.toFixed(2)} ≤ floor $${maxDDFloor.toFixed(2)} (${maxDDPct}% max DD)`,trades);return}
      if(equity<=dailyFloor){firedRef.current=true;cbRef.current(`Daily drawdown breached — equity $${equity.toFixed(2)} ≤ daily floor $${dailyFloor.toFixed(2)} (${dailyDDPct}% daily DD)`,trades);return}
      for(const t of trades){
        const cur=prices[t.symbol]||SEED[t.symbol]||t.open_price
        const pct=(calcPnl(t,cur)/startBal)*100
        if(pct<=-5){firedRef.current=true;cbRef.current(`${t.symbol} trade loss ${pct.toFixed(2)}% of account (limit -5%)`,trades);return}
      }
    },500)
...
export function PlatformPage(){
  ...
  const handleBreach=useCallback(async(reason:string,trades:any[])=>{
    ...
    const closed=await Promise.all(trades.map(async t=>{
      ...
      const isJpy=['USD/JPY','GBP/JPY','EUR/JPY'].includes(t.symbol)
  ...
  async function placeOrder(){
    ...
    const {data,error}=await supabase.from('trades').insert({
      ...
      order_type:ordType.toLowerCase(),open_price:execPrice,sl:sl?+sl:null,tp:tp?+tp:null,
    ...
    <div style={{width:210,flexShrink:0,background:'var(--bg2)',borderLeft:'1px solid var(--bdr)',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:12,display:'flex',flexDirection:'column',gap:8}}>
        ...
        <button onClick={()=>setConfirm(true)} disabled={!canTrade||placing||reqMargin>freeMargin} style={{width:'100%',padding:'12px 0',fontSize:11,letterSpacing:2,textTransform:'uppercase' as const,fontWeight:'bold',cursor:canTrade&&!placing&&reqMargin<=freeMargin?'pointer':'not-allowed',border:'none',opacity:canTrade&&!placing&&reqMargin<=freeMargin?1:0.35,background:dir==='buy'?'var(--green)':'var(--red)',color:dir==='buy'?'#000':'#fff'}}>
          {!marketStatus.open?'Market Closed':placing?'Placing…':`${dir.toUpperCase()} ${lotsNum} ${sym}`}
        </button>
      </div>
    </div>
  </div>
