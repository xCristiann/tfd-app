...
function buildEmailHtml(data: { heading: string; body: string; cta_text?: string; cta_url?: string; recipient_name?: string }): string {
...
<body style="margin:0;padding:0;background:#07090f;font-family:'Inter',Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#07090f;padding:40px 20px;">
    <tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      <tr><td style="padding:0 0 32px;text-align:center;"><span style="font-size:17px;font-weight:800;color:#eef0f6;">TheFunded<span style="color:#00e5a0;">Diaries</span></span></td></tr>
      <tr><td style="background:#0c0f1a;border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:40px;">
        ...
        ${cta_text && cta_url ? `<table cellpadding="0" cellspacing="0" style="margin:0 0 28px;"><tr><td style="background:#00e5a0;border-radius:9px;padding:13px 28px;"><a href="${cta_url}" style="color:#04120c;font-size:14px;font-weight:800;text-decoration:none;">${cta_text} &rarr;</a></td></tr></table>` : ''}
        <div style="height:1px;background:rgba(255,255,255,0.07);margin:28px 0;"></div>
        <p style="margin:0;font-size:12px;color:#4e5568;line-height:1.6;">&copy; 2026 TheFundedDiaries &middot; Independent prop firm comparison</p>
      </td></tr>
    </table></td></tr>
  </table>
</body></html>`
    ...
    const results = await Promise.allSettled(
      emails.map(({ email, name }) =>
        fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({ from: `TheFundedDiaries <${fromAddress}>`, to: [email], subject, html: buildEmailHtml({ heading, body: emailBody, cta_text, cta_url, recipient_name: name }) }),
        })
      )
    )
