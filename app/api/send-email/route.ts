...
const FROM_EMAIL = process.env.FROM_EMAIL || 'hello@thefundeddiaries.com'
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        <tr><td style="padding:0 0 32px 0;text-align:center;">
          <span style="font-size:17px;font-weight:800;color:#eef0f6;">TheFunded<span style="color:#00e5a0;">Diaries</span></span>
        </td></tr>
        <tr><td style="background:#0c0f1a;border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:40px;">
          ...
          ${cta_text && cta_url ? `<table cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
            <tr><td style="background:#00e5a0;border-radius:9px;padding:13px 28px;">
              <a href="${cta_url}" style="color:#04120c;font-size:14px;font-weight:800;text-decoration:none;">${cta_text} →</a>
            </td></tr>
          </table>` : ''}
          ...
          <p style="margin:0;font-size:12px;color:#4e5568;line-height:1.6;">
            ...
            © 2026 TheFundedDiaries · Independent prop firm comparison
          </p>
        </td></tr>
      </table>
    </td></tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { subject, heading, emailBody, cta_text, cta_url, recipients } = body
    ...
    const results = await Promise.allSettled(
      emails.map(({ email, name }) =>
        fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: `TheFundedDiaries <${FROM_EMAIL}>`,
            to: [email],
            subject,
            html: buildEmailHtml({ subject, heading, body: emailBody, cta_text, cta_url, recipient_name: name }),
          }),
        })
      )
    )
    ...
    await adminSupabase.from('email_logs').insert({
      subject, template: 'custom', recipients_count: sent, sent_by: user.id,
