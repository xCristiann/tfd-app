      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '56px 32px 80px' }}>
        <div style={{ marginBottom: '40px' }}>
          ...
          <p style={{ fontSize: '15px', color: 'var(--t2)' }}>
            {firms.length} verified discount codes &middot; sorted by best offer first &middot; apply directly at checkout
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {firms.map(firm => (
              <div key={firm.id} style={{ display: 'grid', gridTemplateColumns: '120px 1fr auto', alignItems: 'stretch', background: 'var(--bg1)', border: '1px solid rgba(167,139,250,0.25)', borderRadius: '16px', overflow: 'hidden' }}>

                {/* DISCOUNT BADGE */}
                <div style={{ background: 'linear-gradient(135deg,#ec4899,var(--violet))', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px', position: 'relative' }}>
                  {firm.promo_label && (
                    <div style={{ position: 'absolute', top: '8px', left: '50%', transform: 'translateX(-50%)', fontSize: '8px', fontWeight: 800, color: '#fff', background: 'rgba(255,255,255,0.2)', padding: '2px 7px', borderRadius: '100px', letterSpacing: '.04em', whiteSpace: 'nowrap' }}>
                      ✦ {firm.promo_label}
                    </div>
                  )}
                ...
                <div style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '14px', borderLeft: '1px solid var(--border)', minWidth: 0 }}>
                  <FirmLogo name={firm.name} logoUrl={firm.logo_url} size={48} radius={12} />
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '13px', color: 'var(--t2)', lineHeight: 1.5 }}>
                      {firm.short_description || `${firm.promo_discount} off &mdash; use code at checkout`}
                    </div>
                  </div>
                </div>
