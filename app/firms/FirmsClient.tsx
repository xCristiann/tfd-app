...
export default function FirmsClient() {
  ...
  useEffect(() => {
    ...
    supabase.from('firms').select('*, challenges(*)').eq('is_published', true)
      .then(({ data }) => { setFirms((data as FirmWithChallenges[]) || []); setLoading(false) })
  }, [])
  ...
  const getLowest = (f: FirmWithChallenges) => f.challenges?.sort((a, b) => a.price_usd - b.price_usd)[0]

  const sorted = [...filtered].sort((a, b) => {
    let av = 0, bv = 0
    if (sortCol === 'trust_score') { av = a.trust_score; bv = b.trust_score }
    if (sortCol === 'price') { av = getLowest(a)?.price_usd || 999999; bv = getLowest(b)?.price_usd || 999999 }
    if (sortCol === 'profit_split') { av = parseInt(getLowest(a)?.profit_split || '0'); bv = parseInt(getLowest(b)?.profit_split || '0') }
    ...
    <>
      ...
      <main style={{ maxWidth: '1300px', margin: '0 auto', padding: '48px 32px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '36px' }}>
          {pills.map(p => (
            <button key={p.key} onClick={() => setMarket(p.key as typeof market)} style={{
              padding: '9px 22px', borderRadius: '100px', fontSize: '14px', fontWeight: 600, cursor: 'pointer',
              ...
              {p.label}
              {p.key === 'crypto' && <span style={{ marginLeft: '6px', fontSize: '9px', fontWeight: 800, padding: '2px 6px', borderRadius: '4px', background: 'rgba(255,255,255,0.25)', letterSpacing: '.05em' }}>NEW</span>}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '40px 2fr 100px 80px 90px 80px 80px 100px 110px 120px', padding: '13px 20px', background: 'var(--bg2)', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--t3)', gap: '8px', alignItems: 'center' }}>
              <div>#</div>
              <div>Firm</div>
              <div style={{ cursor: 'pointer' }} onClick={() => handleSort('trust_score')}>Trust <SortIcon col="trust_score" /></div>
              <div>Steps</div>
              <div>Ph1 Target</div>
              <div>Daily DD</div>
              <div>Max DD</div>
              <div style={{ cursor: 'pointer' }} onClick={() => handleSort('profit_split')}>Split <SortIcon col="profit_split" /></div>
              <div>Payout</div>
              <div style={{ cursor: 'pointer', textAlign: 'right' }} onClick={() => handleSort('price')}>Price <SortIcon col="price" /></div>
            </div>
            ...
            {sorted.map((firm, i) => {
              ...
              return (
                <Link key={firm.id} href={`/firms/${firm.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '40px 2fr 100px 80px 90px 80px 80px 100px 110px 120px', padding: '16px 20px', borderTop: '1px solid var(--border)', gap: '8px', alignItems: 'center', transition: 'background .15s', cursor: 'pointer' }}
                    ...
                    <div>
                      <div style={{ fontSize: '14px', fontWeight: 800, color: trustColor(firm.trust_score), marginBottom: '3px' }}>{firm.trust_score}/100</div>
